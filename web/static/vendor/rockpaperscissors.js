...
Elm.RockPaperScissors.make = function (_elm) {
   "use strict";
   _elm.RockPaperScissors = _elm.RockPaperScissors || {};
   if (_elm.RockPaperScissors.values)
   return _elm.RockPaperScissors.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   var joinable = function (players) {
      return _U.cmp($List.length($Dict.values(players)),
      2) < 0;
   };
   var arenaButton = function (g) {
      return joinable(g.players) ? A2($Html.button,
      _L.fromArray([]),
      _L.fromArray([$Html.text("Join")])) : A2($Html.span,
      _L.fromArray([$Html$Attributes.$class("notice")]),
      _L.fromArray([$Html.text("Already full")]));
   };
   var arena = function (g) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("game")]),
      _L.fromArray([arenaButton(g)]));
   };
   var gameList = F2(function (address,
   model) {
      return function () {
         var listOfGames = $Dict.values(model.games);
         return A2($Html.div,
         _L.fromArray([$Html$Attributes.$class("list")]),
         $List.map(arena)($Dict.values(model.games)));
      }();
   });
   var model = {_: {}
               ,games: $Dict.fromList(_L.fromArray([{ctor: "_Tuple2"
                                                    ,_0: "242782e2-7cd7-4783-be6b-afbab2f7e963"
                                                    ,_1: {_: {}
                                                         ,players: $Dict.fromList(_L.fromArray([{ctor: "_Tuple2"
                                                                                                ,_0: "9e055dd0-af85-4e30-b371-dc8f6057035b"
                                                                                                ,_1: {_: {}
                                                                                                     ,move: $Maybe.Nothing
                                                                                                     ,uuid: "9e055dd0-af85-4e30-b371-dc8f6057035b"}}]))
                                                         ,uuid: "242782e2-7cd7-4783-be6b-afbab2f7e963"
                                                         ,winner: $Maybe.Nothing}}]))
               ,playerUuid: $Maybe.Nothing};
   var log = function (a) {
      return A2($Debug.log,"a",a);
   };
   var gameCreated = Elm.Native.Port.make(_elm).inboundSignal("gameCreated",
   "Maybe.Maybe RockPaperScissors.JsGame",
   function (v) {
      return v === null ? Elm.Maybe.make(_elm).Nothing : Elm.Maybe.make(_elm).Just(typeof v === "object" && "uuid" in v && "winner" in v && "players" in v ? {_: {}
                                                                                                                                                             ,uuid: typeof v.uuid === "string" || typeof v.uuid === "object" && v.uuid instanceof String ? v.uuid : _U.badPort("a string",
                                                                                                                                                             v.uuid)
                                                                                                                                                             ,winner: v.winner === null ? Elm.Maybe.make(_elm).Nothing : Elm.Maybe.make(_elm).Just(typeof v.winner === "string" || typeof v.winner === "object" && v.winner instanceof String ? v.winner : _U.badPort("a string",
                                                                                                                                                             v.winner))
                                                                                                                                                             ,players: typeof v.players === "object" ? {_: {}} : _U.badPort("an object with fields ``",
                                                                                                                                                             v.players)} : _U.badPort("an object with fields `uuid`, `winner`, `players`",
      v));
   });
   var createdGames = A2($Signal.map,
   log,
   gameCreated);
   var JsGame = F3(function (a,
   b,
   c) {
      return {_: {}
             ,players: c
   ...
   var StartNewGame = {ctor: "StartNewGame"};
   var pageHeader = function (address) {
      return A2($Html.header,
      _L.fromArray([]),
      _L.fromArray([A2($Html.h1,
                   _L.fromArray([]),
                   _L.fromArray([$Html.text("Available games")]))
                   ,A2($Html.button,
   ...
   model) {
      ...
      _L.fromArray([$Html$Attributes.id("games")]),
      _L.fromArray([pageHeader(address)
                   ,A2(gameList,address,model)]));
   });
   ...
   _elm.RockPaperScissors.values = {_op: _op
                                   ,Player: Player
                                   ,Game: Game
                                   ,Model: Model
                                   ,NoOp: NoOp
